...
class ScratchCardPileReader
  ...
  def parse_raw_card_values(original_card)
    ...
    @card_stack[@id] = { :copies => 1, :winning_numbers => winning_number_list.split, :card_values => card_numbers.split } 
  end
  ...
  def evaluate_cards
    @card_stack.each do |card_number, card_info|
      ...
      card_info[:copies].times do
        ...
        win_count.each.with_index do |winning_card, i|
  ...
  def count_card_stack
    @card_stack.sum { |card_number, card_info| card_info[:copies] }
  end
end
